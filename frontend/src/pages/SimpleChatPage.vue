...
async function sendMessage() {
  ...
  try {
    const res = await chatApi.simpleChat(text)
    messages.value.push({
      id: generateId(),
      role: 'assistant',
  ...
  <div class="flex flex-col h-full">
    ...
    <header class="h-16 glass-effect border-b border-white/5 flex items-center px-8 gap-4 relative z-10">
      <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-primary-500/20 to-accent/20 flex items-center justify-center">
        <MessageCircle class="w-4 h-4 text-primary-400" />
      </div>
      <h1 class="text-white font-semibold text-base">简单对话</h1>
      <span class="text-xs text-white/30 bg-white/5 px-2 py-0.5 rounded-md">Chat</span>
    </header>
    ...
    <div ref="chatContainer" class="flex-1 overflow-y-auto scrollbar-thin py-6">
      ...
      <div v-if="messages.length === 0" class="flex flex-col items-center justify-center h-full">
        <div class="w-20 h-20 rounded-3xl bg-gradient-to-br from-primary-500/10 to-accent/10 flex items-center justify-center mb-6 shadow-glow">
          <MessageCircle class="w-10 h-10 text-primary-400/60" />
        </div>
        <p class="text-white/50 text-lg font-medium">开始简单对话</p>
        <p class="text-white/25 text-sm mt-2">无需知识库，直接向 AI 提问</p>
      ...
      <!-- 加载指示 -->
      <div v-if="isLoading" class="flex gap-4 px-6 py-3 animate-fade-in">
    ...
    <div class="glass-effect border-t border-white/5 p-5 relative z-10">
      <div class="flex items-end gap-4 max-w-4xl mx-auto">
        <textarea
          ...
          rows="1"
          class="flex-1 bg-white/5 border border-white/10 rounded-2xl px-5 py-3.5 text-sm text-white
